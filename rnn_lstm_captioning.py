...
def temporal_softmax_loss(x, y, ignore_index=None):
    ...
    ##########################################################################
    # Replace "pass" statement with your code
    loss = F.cross_entropy(x.permute(0, 2, 1), y, ignore_index=ignore_index,
...
class CaptioningRNN(nn.Module):
    ...
    def __init__(
        ...
        self.rnnInUse = None
        if cell_type == "rnn":
            self.rnnInUse = self.rnn
        elif cell_type == "lstm":
            self.rnnInUse = self.lstm
        else:
            self.rnnInUse = self.attentionLSTM
            
        self.outputProjection = nn.Linear(hidden_dim, vocab_size)
        self.featureProjection = None
        if cell_type == "attn":
            self.featureProjection = nn.Conv2d(self.imageEncoder.out_channels,
                                                hidden_dim, 1)
        else:
            self.featureProjection = nn.Linear(self.imageEncoder.out_channels*4*4,
                                               hidden_dim)
        ######################################################################
        #                            END OF YOUR CODE                        #
        ######################################################################
    ...
    def forward(self, images, captions):
        ...
        h0_A = self.featureProjection(features)
        
        # Step (2): Use a word embedding layer. "embed_words" shape is (N, T, W)
        embed_words = self.wordEmbedding(captions_in)
        
        # Step (3): Use either a vanilla RNN or LSTM (depending on self.cell_type)
        # A vector sized (N, T, H).
        hidden_layers = self.rnnInUse(embed_words, h0_A)
        
        # Step (4): Use a (temporal) affine transformation to compute scores over the vocabulary
        # A vector sized (N, T, V).
        scores = self.outputProjection(hidden_layers)
        
        # Step (5): Use (temporal) softmax to compute loss using captions_out
    ...
    def sample(self, images, max_length=15):
        ...
        next_h = self.featureProjection(features)
        A = next_h
        next_c = torch.zeros_like(next_h)[:, 0]
        if self.cell_type == "attn":
            next_c = A.mean(dim=(2, 3))
            next_h = next_c
        next_word = torch.tensor([self._start]*N)
        
        
        # Start by makeing the first step of the RNN with the <START> token
        for t in range(max_length):
            embedded_words = None
            # (1) Embed the previous word using the learned word embeddings
            embedded_words = self.wordEmbedding(next_word)  
            
            # (2) Make an RNN step
            if self.cell_type == "attn":
                attn, attn_weights_all[:, t, :, :] = dot_product_attention(next_h, A)
                next_h, next_c = self.rnnInUse.step_forward(
                    embedded_words, next_h, next_c, attn
                )
            elif self.cell_type == "lstm":
                next_h, next_c = self.rnnInUse.step_forward(embedded_words, next_h, next_c)
            else:
                next_h = self.rnnInUse.step_forward(embedded_words, next_h)

            # (3) Apply the learned affine transformation to the next hidden state
            affined_scores = self.outputProjection(next_h)
            
            # (4) Select the word with the highest score as the next word
            next_word = affined_scores.argmax(dim=1)
...
class LSTM(nn.Module):
    ...
    def forward(self, x: torch.Tensor, h0: torch.Tensor) -> torch.Tensor:
        ...
        ######################################################################
        hn = None
        hn = torch.zeros(x.shape[0], x.shape[1], h0.shape[1],
...
def dot_product_attention(prev_h, A):
    ...
    ##########################################################################
    # Replace "pass" statement with your code
    attn_weights = prev_h.view(N, 1, H) @ A.view(N, H, -1)/math.sqrt(H)
    
    attn_weights = attn_weights.view(N, -1)
    attn_weights = F.softmax(attn_weights, dim=1)
    A_tag = A.view(N, H, -1)
    attn = torch.bmm(A_tag, attn_weights.unsqueeze(2)).squeeze(2)
...
class AttentionLSTM(nn.Module):
    ...
    def forward(self, x: torch.Tensor, A: torch.Tensor):
        ...
        for t in range(x.shape[1]):
            next_h, next_c = self.step_forward(x[:, t, :], next_h, next_c, attn)
            attn, _ = dot_product_attention(next_h, A)
            hn[:, t, :] = next_h
        ######################################################################
        #                           END OF YOUR CODE                         #
